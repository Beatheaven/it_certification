import React, { Component } from 'react';
import PropTypes from 'prop-types';
import classname from 'classnames';
import moment from 'moment';
import { TextBody, Radio, Loading, Links } from 'components/atoms';
import { ScoreSurvey, ContentCardSurveyMedium } from 'components/molecules';

import { DocumentUmkmSmart } from 'pdf/molecules';
import { PdfGenerator } from 'pdf/organisms';

import { getHistorySurvey } from '../../../services';

// style
import './styles.scss';

class CardCategoryKompetensiMedium extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
      currentHistory: null,
      date: '',
      category: '',
      score: 0,
      name: '',
      omzet: '',
      business: '',
      dataList: [{ group: 'Skala Usaha', score: 0, max_score: 10 }],
      canvas: null,
      isDownload: false
    };
  }

  componentDidMount() {
    const { historyIds } = this.props;
    if (historyIds.length) {
      this.setState({ currentHistory: historyIds[0] });
      this.getHistorySurvey(historyIds[0]);
    }
  }

  componentDidUpdate(prevprevProps, prevState, snapshotState) {
    const { canvas } = this.state;
    if (prevState.canvas !== canvas && canvas) {
      setTimeout(() => {
        this.setState({ canvas: '' });
      }, 1000);
    }
  }

  getHistorySurvey = (id) => {
    this.setState({ loading: true });
    getHistorySurvey(id)
      .then((response) => {
        const {
          created_at: date,
          umkm_category: category,
          average_score: score,
          business_name: business,
          full_name: name,
          omzet,
          summary
        } = response.data;
        const dateFormat = moment(date).format('DD MMMM YYYY');
        this.setState({
          date: dateFormat,
          category,
          score,
          business,
          name,
          omzet,
          dataList: summary,
          loading: false
        });
      })
      .catch((err) => {
        throw err;
      });
  };

  activeHistory = () => {
    const { currentHistory } = this.state;
    const { historyIds } = this.props;
    return historyIds.map((id, index) => (
      <Radio
        className="mr-16"
        key={id}
        id={id}
        name="activeHistory"
        isChecked={currentHistory === id}
        value={currentHistory}
        isMultipleChoice
        onChange={this.changeActiveHistory(id)}
      >
        <TextBody>
          {index === 0 ? 'Hasil survey terbaru' : 'Hasil survey sebelumnya'}
        </TextBody>
      </Radio>
    ));
  };

  changeActiveHistory = (id) => (e) => {
    this.setState({ currentHistory: id });
    this.getHistorySurvey(id);
  };

  render() {
    const {
      date,
      category,
      score,
      business,
      name,
      omzet,
      dataList,
      currentHistory,
      loading,
      canvas,
      isDownload
    } = this.state;

    const dataChart = dataList.map((data, index) => ({
      subject: `${index + 1}`,
      A: Math.round(data.score),
      fullMark: data.max_score
    }));

    const id = `chart-${currentHistory}`;

    const dataPdf = {
      category,
      score,
      date,
      name,
      business,
      omzet
    };

    const classNames = classname('o-card-category-kompetensi medium', {});
    const classContent = classname('o-card-category-kompetensi__content');
    const classHeader = classname('o-card-category-kompetensi__header');

    return (
      <div className={classNames}>
        <Loading loading={loading} />
        <div className={classHeader}>
          <div className="header-item">
            <TextBody
              weight="regular"
              color="black"
            >{`Berdasarkan hasil survey ${date}`}</TextBody>
            <ScoreSurvey
              category={category}
              nameUmkm={`${omzet} ${category}`}
            />
          </div>
          <div className="header-item text-right">
            {dataChart.length && dataList.length && canvas ? (
              <PdfGenerator
                link
                linkLabel="UNDUH SERTIFIKAT"
                filename="Sertifikat Kompetensi"
                document={
                  <DocumentUmkmSmart
                    data={dataPdf}
                    dataChart={dataChart}
                    dataLegend={dataList}
                    imageChart={canvas}
                  />
                }
              />
            ) : (
              <Links
                underline
                variant="bold"
                color="red"
                onClick={(e) => {
                  e.preventDefault();
                  this.setState({
                    isDownload: true
                  });
                }}
              >
                UNDUH SERTIFIKAT
              </Links>
            )}
          </div>
        </div>
        <div className={classContent}>
          <ContentCardSurveyMedium
            dataPdf={dataPdf}
            dataChart={dataChart}
            dataLegend={dataList}
            infoChart={this.activeHistory()}
            currentData={currentHistory}
            idChart={id}
            setCanvas={(val) =>
              this.setState({ canvas: val, isDownload: false })
            }
            handleDownloadPdf={isDownload}
          />
        </div>
      </div>
    );
  }
}

CardCategoryKompetensiMedium.propTypes = {
  historyIds: PropTypes.array
};

CardCategoryKompetensiMedium.defaultProps = {
  historyIds: []
};

export default CardCategoryKompetensiMedium;
